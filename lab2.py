...
def a_star_search(start_state):
    ...
    visited_states.add(str(start_state))  # Mark the start state as visited

    while open_list:
        # Debug: Print the priority queue before popping
        print("Current states in the heap :  *************************")
        for item in open_list:
            state_str = str(item[1])
            if state_str not in visited_states:
                color_start = Fore.GREEN  # Green for new states
            else:
                color_start = ""

            print(f"{color_start}f_value: {item[0]}, state:")
            for row in item[1]:
                print(f"{color_start}{row}")
        ...
        for neighbor in get_neighbors(current):
            ...
            if str(neighbor) not in g_score or tentative_g_score < g_score[str(neighbor)]:
                ...
                visited_states.add(str(neighbor))  # Mark this state as visited
    return None
